...
function defineVisitor ({
  ...
  return {
    ...
    'ObjectPattern[typeAnnotation]' (node) {
      processNodeList(node.properties, tokenStore.getFirstToken(node), tokenStore.getTokenBefore(node.typeAnnotation), 1)
    },
    /**
    ...
    'ArrayPattern[typeAnnotation]' (node) {
      processNodeList(node.elements, tokenStore.getFirstToken(node), tokenStore.getTokenBefore(node.typeAnnotation), 1)
    },
    /**
     * Override the process of the ClassDeclaration and ClassExpression with superTypeParameters
     */
    'ClassDeclaration[superTypeParameters], ClassExpression[superTypeParameters]' (node) {
      if (node.superClass != null) {
        const superClassToken = tokenStore.getFirstToken(node.superClass)
        const superTypeToken = tokenStore.getFirstToken(node.superTypeParameters)
    ...
    'ClassProperty, TSAbstractClassProperty' (node) {
      ...
      if (node.value != null) {
        const equalToken = tokenStore.getTokenAfter(node.typeAnnotation || lastKeyToken)
    ...
    TSIndexSignature (node) {
      ...
      while (token != null && !isLeftBracket(token)) {
      ...
      const paramLeftToken = token
      const paramRightToken = tokenStore.getTokenAfter(
        node.parameters.length > 0 ? last(node.parameters) : paramLeftToken,
        isRightBracket
      )

      if (hasPrefix) {
        setOffset(paramLeftToken, 0, last(prefixTokens))
      }
    ...
    TSConditionalType (node) {
      ...
      const firstToken = tokenStore.getFirstToken(node)
      const questionToken = tokenStore.getTokenAfter(node.extendsType, isNotRightParen)
      const consequentToken = tokenStore.getTokenAfter(questionToken)
      const colonToken = tokenStore.getTokenAfter(node.trueType, isNotRightParen)
    ...
    TSInterfaceDeclaration (node) {
      ...
      if (node.extends != null) {
        const extendsToken = tokenStore.getTokenAfter(node.typeParameters || node.id, isExtendsKeyword)
    ...
    'TSConstructorType, TSConstructSignatureDeclaration' (node) {
      ...
      if (node.typeParameters != null) {
        ...
        leftParen = tokenStore.getTokenAfter(node.typeParameters, isLeftParen)
      } else {
        leftParen = tokenStore.getTokenAfter(newToken, isLeftParen)
      }

      setOffset(leftParen, 1, newToken)
      const rightParen = tokenStore.getTokenAfter(last(node.params) || leftParen, isRightParen)
      processNodeList(node.params, leftParen, rightParen, 1)

      const arrowOrColonToken = tokenStore.getTokenAfter(rightParen, (node) => isArrow(node) || isColon(node))
    ...
    TSDeclareFunction (node) {
      ...
      const starToken = node.generator ? tokenStore.getTokenBefore(idToken) : null

      let token = tokenStore.getFirstToken(node)
      while (token != null && token.range[0] < (starToken || idToken).range[0]) {
        prefixTokens.push(token)
        token = tokenStore.getTokenAfter(token)
      }
    ...
    TSEnumDeclaration () { console.log('not impl TSEnumDeclaration') },
    TSEnumMember () { console.log('not impl TSEnumMember') },
    TSExportAssignment () { console.log('not impl TSExportAssignment') },
    TSExternalModuleReference () { console.log('not impl TSExternalModuleReference') },
    TSImportType () { console.log('not impl TSImportType') },
    TSInferType () { console.log('not impl TSInferType') },
    TSLiteralType () { console.log('not impl TSLiteralType') },
    TSIndexedAccessType () { console.log('not impl TSIndexedAccessType') },
    TSImportEqualsDeclaration () { console.log('not impl TSImportEqualsDeclaration') },
    TSFunctionType () { console.log('not impl TSFunctionType') },
    TSMethodSignature () { console.log('not impl TSMethodSignature') },
    TSModuleBlock () { console.log('not impl TSModuleBlock') },
    TSModuleDeclaration () { console.log('not impl TSModuleDeclaration') },
    TSNamespaceExportDeclaration () { console.log('not impl TSNamespaceExportDeclaration') },
    TSNonNullExpression () { console.log('not impl TSNonNullExpression') },
    TSMappedType () { console.log('not impl TSMappedType') },
    TSParameterProperty () { console.log('not impl TSParameterProperty') },
    TSQualifiedName () { console.log('not impl TSQualifiedName') },
    TSRestType () { console.log('not impl TSRestType') },
    TSThisType () { console.log('not impl TSThisType') },
    TSTypeAssertion () { console.log('not impl TSTypeAssertion') },
    TSTypeQuery () { console.log('not impl TSTypeQuery') },
    TSIntersectionType () { console.log('not impl TSIntersectionType') },
    TSTupleType () { console.log('not impl TSTupleType') },
    TSOptionalType () { console.log('not impl TSOptionalType') },
    TSParenthesizedType () { console.log('not impl TSParenthesizedType') },
    TSUnionType () { console.log('not impl TSUnionType') }
